...
- 类型在 `src/types/wakenConfig.ts`,配置值在 `src/config/wakenConfig.ts`,两个 barrel(`src/types/config.ts`、`src/config/index.ts`)都已转出
- 小组件在 `src/components/widget/WakenStatus.astro`
- 三个开关的实际效果:
  - `wakenConfig.enable=false` → 导航栏「链接」下的 Waken-wa 入口消失,侧栏卡片也消失
  - `wakenConfig.nowWidget.enable=false`(总开关仍开) → 仅侧栏卡片消失,导航入口保留
  - `wakenConfig.nowWidget.side` 为 `left` / `right` / `both` → 侧栏卡片注入到对应一侧,由 `src/config/sidebarConfig.ts` 顶部的 `buildWakenWidget()` 工厂在构建期决定
- 移动端底部组件**不会**自动注入 Waken 卡片(移动端无左右概念)
