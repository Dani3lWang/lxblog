...
const fieldSets = {
  ...
  moments: [
    ...
    ["author", "作者", "text"],
    ["avatar", "头像", "url"],
    ["images", "图片链接(每行一个)", "lines"],
    ["tags", "标签", "tags"],
    ["location", "地点", "text"],
    ["device", "设备", "text"],
    ["content", "说说内容", "markdown"],
  ],
  changelog: [
    ["filename", "文件名(不含扩展名)", "text"],
    ["version", "版本号", "text"],
...
function titleOf(item) {
  ...
  if (state.tab === "moments") return item.content?.trim() ? item.content.slice(0, 30) : item.author || "未命名说说";
...
function metaOf(item) {
  ...
  if (state.tab === "moments") {
    return [item.published, item.location, item.device].filter(Boolean).join(" · ");
  }
  if (state.tab === "changelog") {
...
function addItem() {
  ...
  if (state.tab === "posts") {
    ...
    item = { id: "", filename: "new-moment", published: today, author: "", avatar: "", images: [], tags: [], location: "", device: "", content: "" };
  } else {
