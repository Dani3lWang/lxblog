 ...
 * 数据文件：src/content/{posts,bangumi,moments,changelog}
 */
...
const MOMENT_KEYS = ["published", "author", "avatar", "images", "tags", "location", "device"];
...
function readMoments(root) {
	const dir = contentDir(root, "moments");
	return readItemFiles(dir, (data, content) => {
		const item = {
			published: data.published || "",
			author: data.author || "",
			avatar: data.avatar || "",
			images: normalizeLines(data.images),
			tags: normalizeArray(data.tags),
			location: data.location || "",
			device: data.device || "",
			title: content ? content.slice(0, 40).replace(/\n/g, " ") : data.author || "",
			content,
			knownKeys: new Set(MOMENT_KEYS),
		};
		return item;
...
function normalizeMoment(entry = {}) {
	...
	const images = normalizeLines(entry.images);
	...
	for (const [key, value] of Object.entries(entry.extra || {})) {
		if (!MOMENT_KEYS.includes(key)) data[key] = value;
	}
	return {
		id: entry.id || "",
